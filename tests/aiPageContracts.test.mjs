...
test("Life opens as a full-screen Figma-based page with uncropped photo frames", async () => {
  ...
  assert.match(styles, /\.life-film-stage\s*\{[^}]*background:\s*rgba\(229,\s*241,\s*232,\s*\.26\);[^}]*backdrop-filter:\s*blur\(8px\)\s*saturate\(96%\)/s);
...
test("Life gallery uses a clickable organic cue and a Figma-matched glass stage", async () => {
  ...
  assert.match(styles, /\.life-title mark\s*\{[^}]*transparent 0 22%,\s*var\(--life-ink\) 22% 78%,\s*transparent 78% 100%/s);
  assert.match(styles, /\.life-title-word::after\s*\{[^}]*clip-path:\s*inset\(22% 0 22% 0\)/s);
...
test("homepage shortcuts use the supplied objects with purposeful hover motion", async () => {
  ...
  assert.match(source, /function PixelCowCat\(\)/);
...
test("Life gallery uses the approved green glass stage and centered designer highlight", async () => {
  ...
  assert.match(styles, /\.life-film-stage\s*\{[^}]*background:\s*rgba\(229,\s*241,\s*232,\s*\.26\)/s);
  assert.match(styles, /\.life-title mark\s*\{[^}]*transparent 0 22%,\s*var\(--life-ink\) 22% 78%,\s*transparent 78% 100%/s);
  assert.match(styles, /\.life-title-word::after\s*\{[^}]*clip-path:\s*inset\(22% 0 22% 0\);/s);
});
